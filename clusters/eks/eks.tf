module "eks" {
  ...
  # IRSA: creates the cluster OIDC provider so add-on service accounts can
  # assume IAM roles. Karpenter itself uses EKS Pod Identity (see karpenter.tf)
  # with IRSA still enabled on the controller role as a fallback.
  enable_irsa = true
